...
<thinking>
我需要先给出前置知识，然后对比三种架构，再分析为什么Decoder-only在通用语言建模中胜出。
...
然后我需要画图对比三种架构。图要简洁，适配PC和手机。我应该使用TB（从上到下）的布局，因为这样在手机上更容易阅读。
</thinking>
...
<thinking>
图画好了，现在我需要分析为什么Decoder-only在通用语言建模中胜出。
...
然后我需要说明Encoder-Decoder并没有死，它在某些任务上仍然是最优选择。
</thinking>
